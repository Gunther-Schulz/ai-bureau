...
| `quality-gate` (Pattern A; LOCKED Phase 3.6 per `arch/quality-gate.md`) | Quality-gate (Pattern A protocol) consumes sparring-event emissions for axis-2 enforcement; per-shape policy declares quality-gate enforcement of sparring requirements |
...
These events feed quality-gate's axis-2 enforcement per `arch/quality-gate.md` §2 capability category B (per-axis signal ingestion).
...
- **ARCH topics composing with sparring**: `arch/substrate.md` (Surface §B hook registration + §D structured output validation; load-bearing for architecturally-encoded sub-mechanisms; subsumes prior coordination Pattern A topic per `docs/decisions/greenfield-rederivation-pause.md` Step 3); `arch/audit.md` (sparring-event emission; mechanism class peer to sparring); `arch/quality-gate.md` (Pattern A 12+7 third instance LOCKED Phase 3.6; FORMAL STABILITY achieved 3 of 3 Pattern A instances; consumes sparring events for axis-2 enforcement per per-axis signal ingestion Surface §B); `arch/claim-defensibility.md` (Phase 3.5 fourth primitive-cluster LOCKED; sparring fires AT claim granularity per `glossary/sparring.md` composes-with claim row + `arch/claim-defensibility.md` §4 composition table — counter-arguments target individual claims; confidence calibration applies per claim; selective friction triggers per claim ambiguity; per-sub-mechanism sparring events compose into engaged-authorship's production-phase substrate per `arch/claim-defensibility.md` §3 + §2.2 Cond #1 — sparring-event-trail as axis-2 → axis-3 dependency; per-shape activation matrix per §4 mediates per-shape engaged-authorship enforcement per `arch/claim-defensibility.md` §8 6-row matrix); `arch/specialist-skill.md` (Phase 3.5 primitive-cluster; sparring sub-mechanisms invoked from skills within specialists — skills with declared output schemas compose with substrate Surface §D structured output validation per `arch/specialist-skill.md` §2.2 skill atomic structure; per-shape activation matrix per §5 declares which sparring sub-mechanisms apply per skill firing); `arch/practitioner.md` (Phase 3.5 second primitive-cluster; practitioner is engagement subject of sparring per axis-2 cross-axis composition per `arch/practitioner.md` §4; sparring engagement events `actor_kind: ai_runtime` AI sparring-partner compose with practitioner-record per per-shape activation matrix per §14; production-phase substrate for engaged-authorship per `glossary/engaged-authorship.md`); `arch/workflow-work-unit.md` (Phase 3.5 third primitive-cluster; sparring sub-mechanisms accessed by skills DURING workflow_instance phase progression AND ad-hoc work-unit progression; orthogonal to workflow primitive engagement per per-shape activation matrix — sparring fires regardless of workflow_instance presence per `arch/workflow-work-unit.md` §4 composition with sparring); `arch/scope-model.md` (Phase 3.5 first cross-cutting integrator LOCKED — sparring Pattern D mechanism class composes through scope-model per §4 E5 authority-binding placement pattern parallel: sparring Surface (8 sub-mechanism contracts per §4) + per-shape activation matrix definitions at Framework C via shape policy bundle + sparring events captured in audit-trail at workspace at Owner B; per-shape activation matrix per §4 framework-baseline-vs-shape-extension partition composes with `arch/scope-model.md` §8 cross-shape policy variation 6-row matrix substrate selection constraints row; §18 per-primitive composition table sparring row); `arch/axis-interactions.md` (Phase 3.5 sixth + final ARCH topic LOCKED; second cross-cutting integrator extending scope-model anchor WITHOUT variation — sparring Pattern D mechanism class **axis-2 PRIMARY** per §4.1 per-primitive axis-anchoring catalog (8 sub-mechanism categories Surface; per-shape activation matrix per §4 framework-baseline-vs-shape-extension partition); cross-axis dependency on axis-1 per §3.3 asymmetric composition + CC-1 §3.4 (sparring requires intertwined co-work as substrate; axis-1 failure forecloses axis-2); cross-axis output for axis-3 per §3.1 pairwise composition (sparring events ARE production-phase substrate for engaged-authorship Cond #1 per `arch/sparring.md` §1 cross-axis dependency line 19); axis-2 → axis-3 production-phase cascade per CC-1 §3.4 (axis-2 failure cascades to axis-3 production-phase); §18 per-primitive composition table sparring row — Phase 3.5 CLOSED with this lock). Cancelled: `arch/coordination.md` + `arch/trust.md` + `arch/time.md` (subsumed per `docs/decisions/greenfield-rederivation-pause.md` Step 3).
