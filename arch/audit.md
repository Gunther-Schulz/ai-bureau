...
| Quality-gate consumption of audit-event emissions | Out (composes with quality-gate Pattern A LOCKED Phase 3.6 per `arch/quality-gate.md`) |
...
| `quality-gate` (Pattern A; LOCKED Phase 3.6 per `arch/quality-gate.md`) | Quality-gate (Pattern A protocol) consumes audit-event emissions for axis enforcement; per-shape policy declares quality-gate enforcement of audit requirements; gate state-restore reads via Surface §C query API + gate-state events emit via Surface §A per audit-trail-as-state-store reframe |
| `category collapse` | Cross-axis force quality-gate guards against; audit-trail provides the unified event stream quality-gate consumes for cross-axis collapse-detection (axis-1 tacked-on / axis-2 answer-machine-oracle-validator / axis-3 rubber-stamping manifestations all observable via per-shape event-kind catalog emissions) |
...
- **ARCH topics composing with audit**: `arch/substrate.md` (Surface §8 dual-emission converges here; Surface §F provides storage realization; Surface §C permission flow integrates audit with authority-binding mechanism (independent framework primitive; per `MAINTENANCE.md` TOP-LEVEL ARCHITECTURE concept-by-concept table). Prior `trust` Pattern A topic CANCELLED per `docs/decisions/greenfield-rederivation-pause.md` Step 3 cascade — trust model is per-shape policy on authority-binding, not subsumed into audit class. Substrate also subsumes prior `coordination` Pattern A topic per same DR — substrate hooks + event-bus subsume cross-claim audit-trail coordination); `arch/adapter.md` (§8 per-action emission; subsumes prior `time` Pattern A topic per same DR — adapter time-driven operations subsume scheduled audit operations); `arch/sparring.md` (§8 per-sub-mechanism emission; mechanism class peer to audit); `arch/quality-gate.md` (Pattern A 12+7 third instance LOCKED Phase 3.6; FORMAL STABILITY achieved 3 of 3 Pattern A instances; consumes audit-event emissions for axis enforcement; gate-state events flow through Surface §A emission + §C query per audit-trail-as-state-store reframe); `arch/claim-defensibility.md` (Phase 3.5 fourth primitive-cluster LOCKED; Surface §A emission API for §13 6-event-kind catalog candidate per `arch/claim-defensibility.md` §13 — `claim_made` / `claim_revised` / `claim_finalized` / `claim_attested` / `claim_re_attested` / `defensibility_test_run`; Surface §B append-only persistence preserves claim revision per-version semantics per `arch/claim-defensibility.md` §3 + §13; Surface §C query API for cross-claim audit-trail defensibility test reconstruction per `arch/claim-defensibility.md` §2.2 re-run-ability; Surface §D integrity verification for cryptographic signing per `arch/claim-defensibility.md` §14 W4; §10 boot ordering integration per `arch/claim-defensibility.md` §13 boot integration — `claim_made` event-kind catalog must register before substrate emits first audit event; §14 cross-shape policy variation per-shape audit emission granularity composes with claim attribution per `arch/claim-defensibility.md` §8 6-row matrix); `arch/specialist-skill.md` (Phase 3.5 primitive-cluster; specialist-lifecycle event-kind catalog emits via audit Surface — `specialist_activated` / `specialist_deactivated` / `specialist_skill_registered` / `specialist_load_failed` / `specialist_version_bumped` per `arch/specialist-skill.md` §7 catalog; skill firings emit via skill-side MCP audit gate per substrate §8 dual-emission); `arch/practitioner.md` (Phase 3.5 second primitive-cluster; per-event actor declaration via Surface §A records practitioner identity for human-actor events; per-shape audit emission granularity per §14 composes through practitioner-record per `arch/practitioner.md` §4; cross-practitioner audit-trail query pattern via Surface §C query API per `arch/practitioner.md` §4); `arch/workflow-work-unit.md` (Phase 3.5 third primitive-cluster; Surface §A emission API for workflow_instance + work-unit instance event-kind catalog per `arch/workflow-work-unit.md` SD-4 — `workflow_started` / `_phase_transitioned` / `_suspended` / `_resumed` / `_completed` / `_abandoned` / `_failed` + `work_unit_created` / `_state_transitioned` / `_completed` / `_sent` / `_archived` / `_pivoted`; Surface §C query API for cross-workflow_instance + cross-work-unit-instance audit-trail defensibility test reconstruction; §14 cross-shape policy variation per-shape audit emission granularity composes with work-unit attribution); `arch/scope-model.md` (Phase 3.5 first cross-cutting integrator LOCKED — audit Pattern D mechanism class composes through scope-model per §4 E5 authority-binding placement pattern parallel: audit Surface + AuditEvent schema at Framework C + audit-trail at workspace (sequence of events; append-only) at Owner B; per-shape audit emission granularity per §14 composes with `arch/scope-model.md` §8 cross-shape policy variation 6-row matrix substrate selection constraints row (claim-level practitioner-shape / action-level autonomous-business / light personal-OS); §18 per-primitive composition table audit row + authority-binding row (mechanism dual-aspect placement parallel)); `arch/axis-interactions.md` (Phase 3.5 sixth + final ARCH topic LOCKED; second cross-cutting integrator extending scope-model anchor WITHOUT variation — audit Pattern D mechanism class **axis-3 PRIMARY** per §4.1 per-primitive axis-anchoring catalog (audit-trail-as-canonical-source per `arch/audit.md` §10 IS axis-3 defensibility evidence layer — Cond #2 reconstructible reasoning chain depends on append-only audit-trail integrity); cross-axis serves axis-2 (sparring events captured in audit-trail per §A) + axis-1 (event-bus extension per `arch/substrate.md` §E composes with axis-1 trust per `glossary/authority-binding.md`); cross-axis composite boot integration per §3.5 E1 — audit-phase 1-3 axis-3 evidence layer ready BEFORE substrate-phase 1-5 axes 1+2 activate per §10 boot-before-substrate / shutdown-after-substrate ordering invariant; per-shape audit emission granularity per §14 composes with `arch/axis-interactions.md` §8 cross-axis activation matrix; §18 per-primitive composition table audit row — Phase 3.5 CLOSED with this lock). Cancelled per `docs/decisions/greenfield-rederivation-pause.md` Step 3 cascade: `arch/coordination.md` (subsumed into substrate hooks + event-bus) + `arch/trust.md` (per-shape trust policy lives at shape-policy declaring trust at authority-binding mechanism) + `arch/time.md` (subsumed into substrate-impl temporal semantics + adapter time-driven operations).
