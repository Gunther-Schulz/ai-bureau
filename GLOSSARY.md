...
- [quality-gate](#quality-gate) — runtime checkpoint validation Protocol; cross-axis category-collapse-resistance enforcement
...
- [engaged authorship](#engaged-authorship) — DERIVED axis-3 success mode (two-phase composite: production-phase sparring + attestation-phase per-claim attestation; success contrast to rubber-stamping)
...
- `engaged authorship` — operational success mode this axis preserves (two-phase composite per-claim test); the architectural commitment this axis expresses, made observable via engaged-authorship's structural signals
...
- `engaged authorship` — per-claim engagement is the success-mode test; per-claim engagement events fire DURING claim production (production-phase) + AT claim finalization (attestation-phase per-claim attestation event); engaged-authorship test resolves at claim granularity
...
- `engaged authorship` — operational definition of defensibility's engaged-authorship condition (two-phase composite per-claim test); per-claim engaged-authorship + per-claim source-grounding + per-claim audit-trail-completeness compose into per-claim defensibility
...
- `engaged authorship` (operational definition of engaged-authorship condition; two-phase composite test)
...
**Composes with**:
- `defensibility` — engaged authorship IS the operational definition of defensibility's engaged-authorship condition (defensibility condition #1); per-claim defensibility composes from per-claim engaged-authorship + per-claim source-grounding + per-claim audit-trail-completeness
- `authorship preservation (axis 3)` — engaged authorship IS the success mode authorship preservation requires (architectural commitment expresses; engaged authorship makes it observable)
- `rubber-stamping` — engaged authorship's attestation-phase failure mode contrast (asymmetric: rubber-stamping is attestation-only failure; engaged authorship is two-phase success requiring both phases)
- `answer-machine AI` / `oracle AI` / `validator AI` — engaged authorship's production-phase failure mode contrasts (each is axis-2 failure that bypasses production-phase engagement)
- `claim` — per-claim granularity; per-claim engagement events fire DURING claim production (production-phase) + AT claim finalization (attestation-phase); aligns with claim lifecycle
- `event` — engagement events compose audit trail; per-claim per-phase event-existence is the framework-PRESENCE test
- `sparring (axis 2)` — production-phase engagement is sparring-anchored; sparring events are the production-phase signal substrate
- `actor` — practitioner-engagement tested via human-actor events (`actor_kind: human`); AI-runtime is sparring substrate, not engagement subject
- `mechanism` — composes with audit-emission (per-phase event capture) + authority-binding (WHO can attest per shape policy) + source-grounding (separate defensibility condition; orthogonal). Authority-binding declares WHO; engaged-authorship declares WHAT attestation captures
- `quality-gate` (Pattern A) — quality-gate's axis-3 intervention applies engaged-authorship test at attestation moments; engaged authorship's quality signals are quality-gate's axis-3 observability source
...
- `category collapse` — engaged-authorship-failure is category-collapse manifestation on axis 2 (production-phase) and axis 3 (attestation-phase)
- `shape` — shape policy declares quality thresholds + intervention semantics; framework declares presence test
- `policy` — practitioner-shape policy mandates engaged-authorship presence + quality intervention; autonomous-business-shape policy mandates programmatic attestation; personal-OS-shape policy mandates light-touch
...
- Locked GLOSSARY entries: `defensibility` (Condition #1: "engaged authorship"); `rubber-stamping` (failure mode at attestation phase only; axis-2 failures + rubber-stamping are INDEPENDENT dimensions); `claim` (per-claim granularity; CREATED during workflow execution + FINALIZED at send/sign moment); `sparring (axis 2)` (production-phase engagement substrate); `quality-gate` (axis-3 intervention applies engaged-authorship test); `authorship preservation (axis 3)` (architectural commitment expressing engaged authorship); `actor` (`actor_kind: human` distinguishes practitioner engagement from AI-runtime sparring substrate)
...
**See**:
- `defensibility` (which engaged authorship is condition #1 of)
- `authorship preservation (axis 3)` (architectural commitment engaged authorship operationalizes)
- `rubber-stamping` (attestation-phase failure mode contrast)
- `answer-machine AI` / `oracle AI` / `validator AI` (production-phase failure mode contrasts)
- `claim` (per-claim granularity)
- `sparring (axis 2)` (production-phase engagement substrate)
- `quality-gate` (Pattern A runtime protocol applying engaged-authorship test at attestation moments)
- ARCH Layer 3 engaged-authorship-detail topics (placeholder until Phase 3 — full event signal catalog per-phase; per-claim-kind variation [interpretive vs citation vs procedural]; quality threshold schema per shape; multi-practitioner co-attestation mechanics; pre-existing-claim ingestion semantics; archived material to consult: `audit-trail-v2.md` for event schema)
...
- **Class**: DERIVED (meta concept naming the relationship between framework and originating deployment; not a primitive with framework-level instances of its own)
- **Layer**: framework-meta (operates on framework + deployments without being one)
- **Axis**: cross-axis (pioneer instance instantiates all axes; isn't itself axis-bound)
- **VISION usage**: not directly named in current VISION (post Phase 1.85 clean-stance restructure relocated pioneer-specific framing to `PIONEER.md`); implicit via VISION's falsification framing — falsification depends on deployment evidence, and pioneer instance provides that evidence base while framework matures

**Canonical**: The originating deployment of a framework — typically the first / seminal implementation — which serves three purposes simultaneously:

1. **Production-tool**: the practitioner who pioneered the deployment uses it for actual professional work (not demo, not testing)
2. **Research-lab**: real work surfaces requirements that drive framework evolution; deployment experience is upstream signal for framework refinement
3. **IP-proving-ground**: the deployment's outcomes (successes / failures / regulatory challenges) validate or falsify framework claims; pioneer evidence is load-bearing while alternative-deployment evidence is sparse

For PBS specifically: PBS-Schulz is the pioneer instance — Gunther Schulz's planning bureau workspace at his Planungsbüro. The framework was extracted FROM that work, then re-applied TO it; refinements flow bidirectionally between framework and pioneer. See `PIONEER.md` for full pioneer-instance identity-anchor.

**What it is**: A meta-concept naming the relationship between framework and originating deployment. Not specific to AI frameworks — applies to any methodology / framework / tool that emerges from real practice. The triple-purpose framing distinguishes pioneer instance from generic deployments: most deployments are JUST production tools; pioneer instances also produce framework evolution feedback + accumulate IP-validating evidence.

The pioneer instance bears load-bearing weight for the framework's empirical validation — claims unsupported by pioneer evidence are speculative until additional deployments accumulate. As alternative deployments multiply, evidence base broadens; pioneer status gradually shifts from primary validator to historical anchor.

**What it is NOT**:
- Not a primitive type — pioneer instance is a ROLE / RELATIONSHIP a workspace takes on, not a kind of entity
- Not specific to AI frameworks — applies to any extractable methodology
- Not implying first-deployment-is-best — pioneers evolve based on later deployments' feedback (pioneer is starting point, not endpoint)
- Not permanent — once second/third deployments accumulate evidence, pioneer instance's role shifts; pioneer status becomes historical context rather than active validation source
- Not synonymous with `workspace` — workspace is the architectural primitive; pioneer instance is the ROLE a workspace plays when serving the originating-deployment function
- Not "the canonical workspace" — there is no canonical workspace; pioneer is just the first/originating one

**Cross-archetype illustration**:

*Pioneer instance manifestations per archetype*:
- **PBS-Schulz** (pioneer of PBS framework): planning bureau deployment; production tool for B-Plan-Begründung work; research lab for framework evolution; proving ground for axis claims (intertwining viability, sparring effectiveness, defensibility under EU regulatory pressure)
- *Hypothetical parallels*: a research lab that pioneers a research-methodology framework; a legal practice that pioneers a legal-AI framework; a creative studio that pioneers a creative-collaboration framework — all would serve same triple-purpose role for their respective frameworks

In all cases: the pioneer instance is where framework theory meets practice; bidirectional refinement happens here; framework claims survive or fail based on pioneer evidence (until evidence base broadens).

**Boundary test**: Three questions — pioneer instance role applies when ALL resolve favorably:
1. Is this deployment serving as a **production-tool** (practitioner uses it for actual professional work, not demo / testing)?
2. Is this deployment serving as a **research-lab** (real work surfaces requirements that drive framework evolution)?
3. Is this deployment serving as an **IP-proving-ground** (deployment's outcomes validate or falsify framework claims)?

If 1+2+3 yes → pioneer instance role applies. If only 1 (production-tool) → regular deployment, not pioneer.

Negative-marker test (deployment is NOT pioneer instance):
- Deployment uses framework but doesn't surface framework-evolution requirements
- Deployment outcomes don't feed back into framework refinement
- Deployment is one of many; evidence base already broadened beyond it
- Framework was developed independently of this deployment
...
- `defensibility` — pioneer regulatory-challenge experience tests defensibility's three structural conditions in real conditions
- `category collapse` — pioneer experience surfaces category-collapse manifestations across all axes + counter-mechanisms that resist them
- `claim` — pioneer claims provide ground-truth for framework's claim primitive (what does an actual claim look like at pioneer scale + complexity?)
- `specialist` — pioneer's specialists inform framework's specialist patterns
- `practitioner` — the role anchored at the pioneer instance (Gunther Schulz for PBS)

**Cardinality + lifecycle**: 

**Cardinality**: typically 1 per framework (the originating deployment). Could be MULTIPLE pioneers if framework emerges from parallel originating efforts where multiple deployments contributed jointly to framework extraction. PBS has 1 pioneer (PBS-Schulz). Multi-pioneer cases are theoretically possible but less common; would require parallel evidence-aggregation discipline.

**Lifecycle**: 
- **Begins**: at framework extraction — when the deployment's patterns crystallize into framework primitives
- **Active phase**: while framework relies primarily on pioneer evidence (alternative deployments sparse or absent)
- **Transition**: as additional deployments accumulate, pioneer status gradually shifts from primary validator to historical anchor; framework's evidence base broadens beyond pioneer's data
- **Mature phase**: pioneer status becomes historical context; framework claims now have multi-deployment evidence base; pioneer no longer load-bearing for individual claims, but remains origin reference
...
**See**:
- `PIONEER.md` (PBS-Schulz pioneer-instance identity + current deployment status + relation to framework)
- `workspace` (the primitive that takes on pioneer-instance role)
- `practitioner-shape` (typical shape for pioneer instances)
- `framework` (what the pioneer instance pioneers)
- VISION axes 1-3 (falsification claims that pioneer evidence tests)
- ARCH Layer 3 pioneer-instance topics (placeholder until Phase 3 — pioneer-instance lifecycle mechanics; transition from active to mature; multi-pioneer cases; pioneer-evidence weighting in framework decisions; per-deployment evidence aggregation)
...
- `engaged authorship` — gate's axis-3 observability source; engaged-authorship's structural signals (production-phase events + per-claim attestation events + quality signals) feed quality-gate's drift detection at attestation moments
- `tacked-on AI / answer-machine AI / oracle AI / validator AI / rubber-stamping` — axis-failure mode manifestations gate detects per axis
- `co-worker` — relational frame quality-gate guards against collapse
- `claim` — gate fires per-claim at finalization (per shape policy granularity)
...
- `sparring (axis 2)` / `audit` protocols (composing protocols)
...
**Composes with**:
- `engaged authorship` — success-mode contrast; rubber-stamping is engaged-authorship's attestation-phase failure mode (asymmetric: rubber-stamping is attestation-only failure; engaged authorship is two-phase success requiring production + attestation)
