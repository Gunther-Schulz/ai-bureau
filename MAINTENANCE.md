...
Before committing any non-trivial doc change: (1) **grep the affected concept name** across all docs (`git grep <term>`) — every hit is a candidate target; (2) **review each hit** — if the change shifts its meaning, update; otherwise verify-consistent in commit message; (3) **cross-check the layer above and below** for required overview / topic updates; (4) **GLOSSARY back-check** — does this work surface a glossary-grade structural fact / reciprocal symmetry / vocabulary distinction needing retro-fit?; (5) **cross-corpus sweep** — when locking a new architectural commitment that introduces a cluster-anchor primitive OR closes a phase-set, run `git grep -l "(forthcoming)" glossary/ arch/` + `git grep -nE "Phase [0-9]\.[0-9]+ forthcoming|canonical entry forthcoming|placeholder until Phase" glossary/ arch/` to detect placeholder framings of OTHER terms whose canonical entry / ARCH topic the just-locked work has now made citable; update placeholders → LOCKED links in same commit (empirically grounded — within-phase cumulative continuity beyond a 3-incident threshold AND cross-phase placeholder carryovers from prior phase-set persisting into the current one are recurring leakage modes that within-cluster cascade-pass alone does not catch); (6) when uncertain, surface the cascade question rather than committing partial cascade.
...
**Portable meta-kits also live here** (sanctioned third category alongside framework artifacts + dev skills): instructional generators distilled FROM observing this repo's dev-process, packaged for OTHER projects to derive their own procedures (e.g., `process-kit/`). Distinct from framework artifacts (PBS-specific architecture; coupled to VISION + GLOSSARY) and from dev skills (sharpening / audit tooling operating IN this repo's dev-process). Co-located here for provenance + ease of update; consulted (not transplanted) by adopter projects per their own derivation discipline. Each portable kit carries its own update protocol (e.g., `process-kit/self-application.md` — kit updates only on fundamental research / model / cross-project-failure triggers, applied via the kit's own derivation procedure on itself); does NOT cascade with framework artifacts. Distinct from `drafts/` (exploratory PBS-specific thinking; never portable; matures to locked PBS docs OR is discarded).
...
- **Two Pattern B clusters** (workflow-work-unit; ANCHOR per `arch/workflow-work-unit.md`): granularity + per-primitive lifecycle ordering APPLIES; cross-shape policy variation APPLIES; bundle / marketplace N/A documented explicitly (workflow + work-unit are bundled IN specialist, not bundlers themselves); 12+5 template extends WITHOUT variation
- **PRIMITIVE + DERIVED clusters** (claim-defensibility; ANCHOR per `arch/claim-defensibility.md`): granularity + per-primitive lifecycle ordering APPLIES; cross-shape policy variation APPLIES (claim-emission audit granularity + engaged-authorship enforcement + per-claim source-grounding strictness + attestation-event mandatoriness + quality-gate signal-set + cross-deployment claim portability all shape-policy-mediated per `arch/audit.md` §14 + engaged-authorship DR Framework-level enforcement); bundle / marketplace N/A documented explicitly (claim is content-level IN work-unit; defensibility is property/test); 12+5 template extends WITHOUT variation
...
- workflow-work-unit (two Pattern B; ANCHOR for two-Pattern-B topic-template-class): 12 common + 3 conditional applies (§8 + §9 + §13) + 2 N/A documented (§10 + §11) + §12 N/A-parity = 18 total per `arch/workflow-work-unit.md` (482 lines)
- claim-defensibility (PRIMITIVE + DERIVED; ANCHOR for PRIMITIVE+DERIVED topic-template-class): 12 common + 3 conditional applies (§8 + §9 + §13) + 2 N/A documented (§10 + §11) + §12 N/A-parity = 18 total per `arch/claim-defensibility.md` (422 lines)
...
Established by `arch/scope-model.md` (cross-cutting integrator ANCHOR for cross-cutting integrator topic-template-class; first cross-cutting integrator instance; closed 5 of 6 Phase 3.5 ARCH topics). Extended by `arch/axis-interactions.md` (Phase 3.5 sixth + final ARCH topic; second cross-cutting integrator instance; extends scope-model anchor WITHOUT variation per `docs/decisions/pattern-a-template-7th-conditional-cross-shape-variation.md` instance-driven trigger pattern; closes Phase 3.5 6 of 6 ARCH topics LOCKED). **Cross-cutting integrator topic-template-class FORMAL STABILITY achieved with second instance (axis-interactions) — 12+5 extends WITHOUT variation across both scope-model anchor + axis-interactions instance per per-pattern instance-driven trigger pattern. Cross-cutting integrator instances complete with Phase 3.5 close (2 of 2).**
...
- **Cross-cutting integrators** (scope-model ANCHOR + axis-interactions first extension; FORMAL STABILITY across 2 of 2 instances): §8 cross-shape policy variation APPLIES (cross-cutting integrator's surface is shape-policy-mediated where shape-policy declares per-shape variation in engagement-target catalog / activation matrix / per-shape variation surface — scope-model engagement-target Owner B managed entities shape-policy-mandated; axis-interactions per-shape cross-axis activation matrix 6-row variation per §8); §9 granularity / §10 bundle / §11 marketplace / §13 per-element lifecycle ordering N/A documented explicitly (cross-cutting integrators have no granularity-test / bundle / marketplace / per-element lifecycle-ordering surface — those concerns live in respective primitive-cluster + Pattern A topics whose entities ARE placed across scopes); §12 N/A-parity preserved per primitive-cluster + Pattern A precedent; 12+5 template extends WITHOUT variation across both instances (1 conditional applies + 4 N/A documented + §12 N/A-parity = 18 sections total)
...
- axis-interactions (cross-cutting integrator; Phase 3.5 sixth + final ARCH topic; extends WITHOUT variation per anchor): 12 common + 1 conditional applies (§8 per per-shape cross-axis activation matrix 6-row variation) + 4 N/A documented (§9 + §10 + §11 + §13) + §12 N/A-parity = 18 total per `arch/axis-interactions.md` (372 lines)
