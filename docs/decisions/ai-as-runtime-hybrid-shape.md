...
| **D2**: Body section conventions for selective AI read (e.g., AI reads only `## When this applies` to know if doctype applies) | Defer until pain felt | Premature optimization — today AI reads full bodies; only formalize section-addressing when entity bodies grow large enough that selective reading saves meaningful context |
