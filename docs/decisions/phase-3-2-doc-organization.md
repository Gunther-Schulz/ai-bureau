...
**Sub-decision 1 (topic taxonomy) amendment**:
- Topic count: 14 → 11
- Pattern A protocol topics: 8 → 3 (substrate / adapter / quality-gate)
- Mechanism-class topics retained as ARCH topics (NEW category): sparring + audit (reclassified from Pattern A; per-shape policy variation, not pluggability)
- Cancelled topics: arch/coordination.md + arch/trust.md + arch/time.md (subsumed into substrate hooks + authority-binding mechanism + substrate-impl temporal semantics / adapter time-driven operations respectively)
- Foundation-up dependency claim updated: topics 1-2 Pattern A + 3-4 mechanism classes + 5 quality-gate Pattern A (3.6) + 6-9 primitive clusters + 10-11 cross-cutting integrators

**Sub-decision 2 (file naming convention)**: UNCHANGED — kebab-case + flat arch/ + no prefixes still hold for 11 retained topics.

**Sub-decision 3 (cross-cutting topics placement)**: UNCHANGED — TOPICS-vs-CONCERNS distinction still holds; quality-gate + scope-model + axis-interactions still cross-cutting topics.
...
**Why this amendment doesn't invalidate the Mode-2 composite decomposition pattern**: the pattern (sub-decision inventory + foundation-up + per-sub-decision 2-round + synthesis) was correctly applied; the AMENDMENT is downstream cascade from a Phase 3.4 audit (greenfield-rederivation), not a defect in the Mode-2 pattern itself. Mode-2 composite-decomposition pattern remains empirically validated.
