...
fallback_when_mcp_absent: "without list_reference_manifests the skill cannot operate — manifests are contract-bearing (last_updated, last_fetched, checksum_sha256 invariants per ARCHITECTURE meta-rule 4 fail-closed corollary). Surface 'MCP unreachable; restart backend' and stop. When the gate is up but search_corpus is unavailable: Grep over <office>/_ai-references/ corpus is acceptable for content discovery (corpus content is not contract-bearing), with worse recall than semantic search."
