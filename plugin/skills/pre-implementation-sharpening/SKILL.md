...
### Post-round self-check (v0.6.0; comprehensive termination framework per sharpen v0.12.0)

At the end of each round, AI applies comprehensive termination criteria per `plugin/skills/sharpen/SKILL.md` v0.12.0 — Layer 1 empirical signals + Layer 2 counter-bias mechanisms + Layer 3 verdict logic + Layer 4 mandatory output + target-type modifier + HIGH-magnitude tier + cost-benefit trajectory. Single-metric verdicts insufficient; surface-coverage is multi-signal.

**Target-type for pre-implementation work**: typically AMENDABLE-IN-FLIGHT (implementation-readiness checklists + DR amendments are revisable per execution evidence; not LOCK-HARD architectural decisions). MECHANICS-OPERATIONAL HIGH findings → soft signal (user-decision); STRUCTURAL-LOAD-BEARING HIGH (the ~10-20% architectural flow-back per skill core mechanic) → C1 fires.
