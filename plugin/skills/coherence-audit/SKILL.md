...
**Post-audit self-check (v0.3.4 update)**: at end of audit, comprehensive termination criteria per `plugin/skills/sharpen/SKILL.md` v0.12.0 — Layer 1 empirical signals + Layer 2 counter-bias mechanisms + Layer 3 verdict logic + Layer 4 mandatory output + target-type modifier + HIGH-magnitude tier + cost-benefit trajectory. Single-metric verdicts insufficient; surface-coverage is multi-signal.

**Target-type for coherence-audit**: SET-LEVEL AUDIT typically maps to LOCK-HARD when audit fires at phase-boundary on locked corpus (revisions cascade hard); AMENDABLE-IN-FLIGHT when running mid-phase as routine health-check (findings can defer to phase-boundary if not load-bearing). Declare per audit scope.
