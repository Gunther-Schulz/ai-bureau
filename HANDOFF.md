...
**AI-as-runtime principle — convergent adoption noted, empirical validation DEFERRED** (session 12 honest framing, surfaced via meta-question on session-12 close-out): MS AF Agent Skills + Anthropic plugins + Strands Skills all converged on SKILL.md hybrid-shape format. This is **convergent industry adoption**, NOT empirical validation of the AI-as-runtime hybrid-shape principle (#16). Convergent adoption suggests reasonable design choice but is NOT proof — could be cargo-culting (per failure-mode-catalog). Actual validation requires: empirical comparison against alternatives (schema-only / prose-only / encoded-rules patterns), measurement of outcomes on real workloads, cross-domain portability testing. Validation triggers (deferred): Phase 1 corpus + first project bind (real workflow); first consulting deployment in different domain (cross-domain portability); alternative ecosystem shipping substantially different shape that succeeds (comparative evidence); Phase 0 item 5 testing methodology (measurement infrastructure). Watch-list item: AI-as-runtime principle empirical validation deferred to post-Phase-1-corpus + first cross-domain deployment.
