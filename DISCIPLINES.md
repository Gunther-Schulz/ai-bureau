...
### 4. Cascade prevention (greenfield-draft + minimize-embedded + cascade-pass)
...
3. **Cascade-pass after locking** — review prior cross-references to the new term; reconcile inconsistencies in the same commit per cascade discipline
...
### 5. No-defer; surface info-gaps as watch-list entries

Never defer. If a decision can't be made today because external information genuinely doesn't exist, surface as **watch-list entry** naming the specific external signal awaited. Watch-list entries have resolution mechanisms; defers languish.

Two tests for "literally cannot decide today" (both must pass; if either fails, decide now):
- **External-information test**: name a SPECIFIC external signal whose absence prevents the decision. "We haven't designed it yet" / "downstream isn't locked when we could lock it now" — fail the test.
- **Effort-asymmetry test**: could we do the design today if we chose? If yes — even if it might be wrong — NOT a chronological gap.

Per memory: `feedback_pattern_not_instance_defers.md`, `feedback_defer_instinct.md`.
