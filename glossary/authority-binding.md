...
- `arch/scope-model.md` (Phase 3.5 first cross-cutting integrator LOCKED — §4 E5 authority-binding placement pattern: mechanism dual-aspect placement = Framework C (mechanism definition home; same shape as substrate Surface + Implementations + audit AuditEvent schema) + Owner B (`actor_kind` + actor identity recorded ON events at audit-trail; per-event actor declaration sub-aspect at Owner B); §18 per-primitive composition table authority-binding row)
