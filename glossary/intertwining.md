...
**Composes with**:
- [tacked-on AI](tacked-on-ai.md) — the failure mode contrasted
- [intertwined AI](intertwined-ai.md) — positive mode (the success state of this axis)
- [co-worker](co-worker.md) — relational claim about AI's mode of participation
- `trust mechanisms` — class of axis-1 mechanisms (collective term; per-mechanism detail in ARCH Layer 3)
- [workflow](workflow.md) — what intertwining intertwines WITH (codified workflow_instance; or ad-hoc work-unit when no codified pattern); axis-1 integration applies whether work follows a codified workflow or runs ad-hoc
- [category collapse](category-collapse.md) — risk to axis 1
...
- `arch/substrate.md` (Phase 3.4 locked — substrate Surface §B hooks + §F session/context realize cross-session axis-1 mechanisms); `arch/workflow-work-unit.md` (Phase 3.5 third primitive-cluster LOCKED — workflow IS what intertwined AI intertwines WITH; axis-1 PRIMARY anchor for workflow); [arch/axis-interactions.md](../arch/axis-interactions.md) — axis-1 PRIMARY anchor per §2.1 + §4.1 per-primitive axis-anchoring catalog; axis-1 FOUNDATIONAL substrate for axes 2+3 per §3.3 asymmetric composition; axis-1 failure (tacked-on) forecloses axes 2+3 per CC-1 cross-axis failure cascade pattern §3.4 (Phase 3.5 sixth + final ARCH topic LOCKED; second cross-cutting integrator extending scope-model anchor WITHOUT variation)
