...
- `arch/scope-model.md` (Phase 3.5 first cross-cutting integrator LOCKED — §3 workspace integration as cross-scope composition WITHIN cluster (workspace IS the binding-instance entity at Owner B integrating Framework C selections + Owner B instances + Layer A scope-resolution); §8 cross-shape policy variation 6-row matrix (workspace.md required fields per shape + workspace-scope managed entity universals + shape-extensions per shape + Layer A scope configuration defaults per shape + specialists_active recommended set per shape + substrate selection constraints per shape); §14 W1 multi-tenant federation watch + W2 cross-deployment claim portability watch + W3 workspace identity persistence schema watch); `arch/practitioner.md` §3 multi-practitioner mechanics + legal-entity workspace context placement at WORKSPACE level
