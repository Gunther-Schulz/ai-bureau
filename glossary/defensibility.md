...
- [arch/claim-defensibility.md](../arch/claim-defensibility.md) — defensibility DERIVED test + 3-condition operational test + claim-granularity resolution + composability + Q1-Q4 boundary tests + re-run-ability (Phase 3.5 fourth primitive-cluster ARCH topic LOCKED; PRIMITIVE+DERIVED topic-template-class anchor; archived material to consult for Phase 6 defensibility test reconstruction spec: `audit-trail-v2.md` for reasoning-chain reconstruction; `governance-and-identity-sourcing.md` for authority binding)
- [arch/axis-interactions.md](../arch/axis-interactions.md) — defensibility cross-axis composition per §4.1 per-primitive axis-anchoring catalog (axis-3 PRIMARY anchor; defensibility IS axis-3 success criterion test resolves at claim granularity AGAINST work-products produced through axes 1+2); axis-3 EVALUATIVE about axes 1+2 OUTPUTS per §3.3 asymmetric composition (defensibility test resolves AT claim granularity AGAINST work-products produced through axes 1+2; axis-3 attestation-phase failure INDEPENDENTLY possible regardless of axes 1+2 success); CC-1 cross-axis failure cascade pattern §3.4 (axis-1 failure forecloses defensibility via no reasoning chain; axis-2 failure cascades to engaged-authorship Cond #1 production-phase failure → defensibility fails per claim) (Phase 3.5 sixth + final ARCH topic LOCKED; second cross-cutting integrator extending scope-model anchor WITHOUT variation)
