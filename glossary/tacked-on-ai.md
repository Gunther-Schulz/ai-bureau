...
- `arch/substrate.md` (Phase 3.4 locked — substrate Surface mechanisms whose absence underwrites tacked-on shape); `arch/workflow-work-unit.md` (Phase 3.5 third primitive-cluster LOCKED — workflow precondition for intertwining); [arch/axis-interactions.md](../arch/axis-interactions.md) — tacked-on-ai as axis-1 failure mode per §2.1 + §4.1 per-primitive axis-anchoring catalog (axis-1 LANDSCAPE DEFAULT); axis-1 failure (tacked-on) → forecloses axes 2+3 per CC-1 cross-axis failure cascade pattern §3.4 (tacked-on AI cannot host genuine sparring → axis-2 forecloses; lacks reasoning chains for reconstructable defensibility → axis-3 forecloses); axis-1 collapse to "AI feature catalog" per §4.3 category-collapse manifestation (Phase 3.5 sixth + final ARCH topic LOCKED; second cross-cutting integrator extending scope-model anchor WITHOUT variation)
