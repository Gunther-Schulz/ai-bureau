...
- [arch/axis-interactions.md](../arch/axis-interactions.md) — engaged-authorship 2-phase composite per-claim test composes cross-axis per §2.3 + §4.1 per-primitive axis-anchoring catalog (axis-3 PRIMARY anchor with axis-2 cross-axis production-phase composition); per-claim cross-axis lifecycle composition per §3.6 E6 (production-phase axis-2 sparring events MUST precede axis-3 attestation event for Cond #1 to hold; reverse ordering fails the test); axis-2 → axis-3 production-phase substrate per §3.1 pairwise composition (sparring events ARE production-phase substrate for engaged-authorship Cond #1) (Phase 3.5 sixth + final ARCH topic LOCKED; second cross-cutting integrator extending scope-model anchor WITHOUT variation)
