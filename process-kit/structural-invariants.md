...
Some failure modes cannot be intercepted by tool-call interception hooks (e.g., PreToolUse in Claude Code) — most notably, assertions in natural-language output (see invariant 8 "Structural-enforcement gap"). For these, structural enforcement comes from procedural patterns observable in artifacts and tool-call sequences. Seven patterns derived from review of multiple agent-style frameworks:

1. **Multi-level status indicators**: replace binary claim/no-claim with gradient levels (e.g., a four-level scale distinguishing not-yet-verified / partially-verified / verified / violation states). Forces gradient grading; "I'm not yet sure" is a first-class status. Single-component evidence is structurally distinguishable from cross-component evidence.
...
5. **Structured disk artifacts as basis records**: tracker files (e.g., a structured YAML or JSON tracker at a known project location) hold verification state per finding. Status is structurally observable; readers can inspect the artifact to see what's at what level. Forces accumulation of evidence over time and makes basis grading auditable post-hoc.

6. **Explicit anti-pattern enumeration**: per-discipline "DO NOT" lists naming common failure modes by name. Examples: "mark a finding as resolved without verification"; "assume pattern understanding = verification"; "defer X when investigation can resolve it." Naming failure modes is a stronger guard than naming only success criteria.

7. **Lifecycle hooks contract**: register procedural hooks at well-defined transition points (artifact creation / before-run / after-run / before-cleanup) with explicit failure semantics per hook (fatal-to-current-attempt vs. logged-and-ignored). Different from invariant 5's main hook concept (which is tool-call-interception-style); this is procedural-lifecycle hooks at known transition points. Particularly useful for: workspace setup/teardown enforcement; pre-condition validation before high-stakes operations; post-condition cleanup that must happen even on failure. Typical four-hook pattern: `after_create` failure is fatal to creation; `before_run` failure is fatal to the current attempt; `after_run` and `before_remove` failures are logged and ignored. The fatal-vs-ignored distinction prevents both silent setup-failures (caught) AND cleanup-failures from blocking forward progress (logged but not fatal).
...
**Basis (this sub-section)**: Patterns derived from review of multiple agent-style frameworks. Two-sample-derived as of kit creation; other agent-style frameworks may surface additional or different patterns. Tagged judgment-call per `meta-rules.md` evidence-tier scale until cross-framework validation accumulates. First adopters' experience refines this pattern set.
