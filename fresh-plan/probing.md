...
**Rule**: every fix, finding, completed-work report, and commit MUST be labeled `[design]` or `[impl]` — or `[design+impl]` **ONLY when the [design] lock and [impl] follow-through are atomic (same git commit)**. If landed in separate commits even within the same session: separate labels, separate commits. The mechanical test (same git commit) prevents drift back toward "I'll just call this combined because it feels combined." Applies to:
