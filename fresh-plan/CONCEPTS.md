...
| substrate | D12 (cap section by D17) | Hosts the agent loop; declares capabilities (core abstract: `hooks`, `skills`, `event-chain`) + extension-registered protocol-named capabilities. |
...
| **B — Reference impl of core** | Generic substrate / shape / adapters / specialist; minimal RAG-via-MCP | **Impl side done; Bref + closure entry remaining.** Workstreams per D36 + D41 + D42: B1 ✅ + B2 ✅ (+ followons ✅) + B2b ✅ + B3 ✅ + B4 ✅ + B5 ✅ + B6 ✅ + B7 ✅ + **B8 end-to-end scenario ✅ (D36 §C closure criterion satisfied: all 8 kinds + 5 core payload subtypes + work-unit lifecycle + state_at(n) replay + B1 validation + chain integrity)**. Remaining: **Bref (per D42; refinement workstream; output D-entry analogous to D34)**, then Phase B closure entry (analog of D25 / D35). |
...
- ~~**D39 out-of-band-state tensions surfaced for end-of-Phase-B refinement**~~ → **closed in Bref (impl-side; Bref output entry will record)**: synthetic composition-change:add events at boot now seed manifest-declared actors (full record on `payload.record`); work-unit creation events carry the full record on `payload.after`; per-event check admits self-attestation for the on-event-added actor / work-unit. Pure `state_at(n)` replay now reproduces both. Per-event projection extended in `apply_event_to_state`. Direct `state.add_actor` / `state.add_work_unit` / `state.transition_work_unit` calls outside the projection have been removed from boot + workspace runtime paths.
- **Phase B refinement workstream — Bref (per D42)** — formalized as a named workstream, running between B8 and Phase B closure entry; output is one substantive D-entry analogous to D34. Currently-tracked deliverables (canonical scope list, will grow as B7 / B8 surface more):
  - ~~Resolve D39 out-of-band-state tensions (manifest-actor seeding + work-unit record carry).~~ **Done — impl-side closure landed; Bref output entry §A will record it.**
  - ~~**D17 capability-vocabulary sharpening — confirmed by B2b**~~ → **closed by D43** (rename `event-streaming` → `event-chain`). B2b authoring surfaced that the prior name `event-streaming` pulled readers toward live-runtime-delta streaming (MS `AgentRunResponseUpdate`) when the actual semantic is event-sourcing / checkpointing (MS `Checkpoint`); D43 renames to `event-chain` (matches D10 vocabulary). The other concept mappings (`hooks` ↔ MS `middleware`; `skills` ↔ MS `tools` / `AIFunction`) are semantically clean — no rename needed there.
  ...
  - ~~boot.py step-3 duplication cleanup~~ — **done in B2b** (replaced with `load_provision_spec` + dispatch via new `load_substrate_from_provision` loader).
  - ~~**Subscriber-dispatch reentrancy / loop semantics**~~ → **closed by D44** (queued-dispatch + drain backstop). Append + projection + chain integrity remain synchronous; subscriber `on_event` fires from a FIFO drain owned by the outermost append call. Nested append (events from `on_event`) just enqueues. Loop backstop (`max_events_per_drain`, default 1000) raises `RuntimeError` with diagnostic on overflow.
  ...
  - ~~**Split `decisions.md` into per-entry files at `decisions/D<NN>-<slug>.md`**~~ → **DONE 2026-05-12** (45 entries → 45 files; `decisions.md` is now the chronological index). Done before the cluster supersedes entries (D46+) so they land as new files rather than monolith edits. Precedent: pbs-bureau's GLOSSARY split (36 per-entry files) + DISCIPLINES split (10 per-discipline files).
...
- ~~Two-substrate parity (D41)~~ → **done in B2b**: MSAgentFrameworkSubstrate stub shipped at `impl/extensions/ms-agent-framework-substrate-ext/`; `Substrate` base extracted; cross-tension on D17 capability naming confirmed + queued for Bref (see refinement deliverables list above).
...
fresh-plan/
  ...
  impl/              <- Phase B reference impl (per D36)
    ...
    extensions/      <- shipped canonical extensions
      inprocess-substrate-ext/         <- B2 substrate (InProcessSubstrate(Substrate))
      ms-agent-framework-substrate-ext/ <- B2b substrate stub (MSAgentFrameworkSubstrate(Substrate); per D41 two-substrate parity)
      ...
      rag-via-mcp-ext/                 <- B7 retrieval-shaped extension (rag-retriever-adapter + rag-specialist; registers retrieval-task; per D38 retrieval composes via existing primitives — no knowledge kind)
```
