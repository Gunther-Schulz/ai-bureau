...
- **Phase B refinement workstream — Bref (per D42)** — formalized as a named workstream, running between B8 and Phase B closure entry; output is one substantive D-entry analogous to D34. Currently-tracked deliverables (canonical scope list, will grow as B7 / B8 surface more):
  ...
  - **D17 capability-vocabulary sharpening — confirmed by B2b**. Concept-name mapping observed in writing the MS Agent Framework substrate stub: `hooks` ↔ MS `middleware`; `skills` ↔ MS `tools` / `AIFunction`; `event-streaming` *conflates* two MS-distinct concepts (`AgentRunResponseUpdate` runtime-deltas streaming vs `Checkpoint` event-sourcing — fresh-plan's D39 + D40 §A is the second, not the first). Bref decides: rename, split (`event-streaming` → `event-stream` + `checkpointing`), or keep with documentation. Concept fit is good; naming leans Claude-flavored.
