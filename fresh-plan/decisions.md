...
- [D55](decisions/D55-extends-d19-specialist-activation-scope-interpretation-layer.md) — Extends D19 — specialist.activation-scope interpretation layer + minimal grammar (2026-05-17)
- [D56](decisions/D56-extends-d13-shape-authority-bindings-additional-constraints.md) — Extends D13 — shape.authority-bindings[].additional-constraints interpretation layer + minimal grammar (2026-05-17)
- [D57](decisions/D57-extends-d7-workspace-composition-configuration-passthrough-semantics.md) — Extends D7 — workspace.composition.*.configuration pass-through semantics (2026-05-17)
- [D58](decisions/D58-extends-d20-d39-work-unit-lifecycle-derivation.md) — Extends D20 + D39 — work-unit.lifecycle.started-at + completed-at reconciliation with event-derivation (2026-05-17)
...
- [D74](decisions/D74-phase-c-real-wire-mcp-server-adapter.md) — Phase C real-wire MCP server adapter (closes C6; closes Phase C closure item (b); validates D21 §generalization — workspace-as-MCP-server; HYBRID pure pattern application of D71 (MCP SDK) + D73 (server-side aggregator) — SKIPPED pre-lock probe) (2026-05-17)
- [D75](decisions/D75-phase-c-real-wire-rag-specialist.md) — Phase C real-wire RAG specialist (closes C7; closes Phase C closure items (c) + (f); ACTIVATES D50 §B.1 SkillExecutionError forward-bar; RESOLVES D50 §D D-5 composition contract as first concrete precedent — WRAP AdapterCallError as SkillExecutionError(category='external-dependency-error') with __cause__ chain — FIRED pre-lock probe) (2026-05-17)
- [D76](decisions/D76-phase-c-aegis-integrity-protocol.md) — Phase C AEGIS integrity protocol (closes C8; closes Phase C closure item (g); ACTIVATES D40 §B integrity-mechanism extension point as canonical first; NEW `IntegrityProtocol` ABC + `AEGISIntegrityProtocol` concrete (SHA-256 + Ed25519 + JCS via rfc8785); NEW FAILURE_CATEGORIES `integrity-violation` (21 → 22); NEW `aegis-protocol-ext/0.1.0`; PersistenceLayer extension + boot.py step 4.5+ integration — FIRED pre-lock probe) (2026-05-17)
- [D77](decisions/D77-phase-c-standards-compatibility-cloudevents-prov.md) — Phase C standards-compatibility (closes C9; CloudEvents 1.0 + W3C PROV-DM/PROV-JSON impl-level engagement per D68 §A5 split; NON-BREAKING export-only contract — fresh-plan event envelope UNCHANGED; NEW module `runtime/standards_export.py` with `to_cloudevents` + `event_chain_to_prov_json` + `write_prov_json` pure converters; NEW `ProvJsonExportAdapter` class introducing EXPORT-ONLY adapter sub-shape (no inbound; no outbound real-wire) as durable forward-bar for future OpenTelemetry / AsyncAPI / Activity Streams export adapters per D24 carry-overs; NEW `prov-json-export-ext/0.1.0/` registers protocol-or-transport identifier `prov-json-export`; NEW pyproject dep `cloudevents>=1.10` (resolves 2.0.0); D48 §D D-3 starter category vocabulary REUSED (transport / protocol-error / unknown); ZERO FAILURE_CATEGORIES delta; ZERO event envelope changes — FIRED pre-lock probe) (2026-05-17)
- [D78](decisions/D78-cref-refinement-workstream-output-phase-c-refinement-complete.md) — Cref refinement workstream output (analog of D62) — Phase C refinement workstream complete (processes 18 `log_for_cref` dispositions + 6 cross-cycle pattern observations + Group-4-style retro-validation R1-R9 finding NO T1 architectural findings; 5 inline impl fixes landed atomically — F3 substrate.py docstring 2-msg-types → 3 + F12 adapter.py:1608 `partition` → `rpartition` correctness fix for dotted binding-ids (latent silent-misrouting bug per `_common.schema.json` instance-identifier admitting dots) + 2 new regression tests + F13 adapter.py build_tools docstring `":"` → `"."` per SEP-986 + F16 adapter.py:1874 remove redundant `list(...)` wrap of `all_events()` + F17 standards_export.py `_REQUIRED_EVENT_FIELDS` narrower-than-canonical BY DESIGN docstring/comment clarification; 349 → 351 tests (+2 F12 regression); ZERO FAILURE_CATEGORIES delta; ZERO new D-entries beyond D78; ZERO new modules; ZERO new pyproject deps; SKIPPED pre-lock probe per D62 §F + D63 §F precedent for pure-synthesis workstream-output entries) (2026-05-17)
- [D79](decisions/D79-phase-c-closure-reference-impl-complete.md) — Phase C closure (reference impl complete; analog of D35 + D63) — Phase C complete; 11 D-entries (D69-D77 nine workstream + D78 Cref + D79 closure); 7/7 D68 §C closure-criterion items MET (with locked scope-cuts per C12 + autopilot experiment); 6 scope-cuts surfaced (SC-1 C3/C4 mockable-only; SC-2 closure items (a)/(b)/(e)/(g) in-process harnesses; SC-3 C9 NON-BREAKING per D77; SC-4 production key management Phase D; SC-5 EU AI Act Article 12 bundle format external-trigger 2026-08-02 — compliance-readiness summary per D68 §D conditional; SC-6 MSAF real-wire Phase E per A3); 6 forward-bars to Phase D / E (FB-1 D45 triad standing; FB-2 D40 §B integrity-protocol extension point; FB-3 D77 EXPORT-ONLY adapter sub-shape; FB-4 D67 numerical-drift discipline; FB-5 D55-D59 §D D-7 Phase D end-to-end; FB-6 D64 + D66 runtime forward-bars); 229 → 351 tests (+122; factor 1.53x; verified via `uv run pytest --collect-only -q`); 22 FAILURE_CATEGORIES (3 NEW Phase C + 6 REUSE); SKIPPED pre-lock probe per D35 §F + D63 §F + D78 §E precedent for pure-synthesis phase-closure entries; **autopilot batch-mode experiment outcome CLEAN** per README §"Outcome assessment" — qualifies for reconciliation `fresh-plan-clippy` → canonical `fresh-plan` + promotes batch-mode autopilot as recognized pattern for spec-driven impl-from-locked-contracts phases (0 T1 findings + 1 real latent bug fixed at Cref F12 + C11 retro-validation deviation documented and acceptable via 3 independent fresh-context passes + BATCH AUTHORIZATION held + C1 branch isolation held) (2026-05-17)
