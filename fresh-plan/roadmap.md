...
| 17 | Specialist cluster supersedes entry | NOT STARTED | Apply D45 triad to specialist path: handle_skill failure shape + on_event exception (composes with deliverable 15). ~D50 candidate (was ~D49 before D49 became the 2026-05-12 sharpen-sweep clarification entry per deliverable 21). |
| 18 | Validation cluster supersedes entry | NOT STARTED | Apply D45 triad to validation path: D30 §4 per-work-unit identity checks (named in spec, never implemented) + B1 collect-all skipping when extensions empty. ~D51 candidate (shifted +1). |
