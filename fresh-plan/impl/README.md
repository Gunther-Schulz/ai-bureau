...
- ~~**D39 out-of-band-state tensions remain at Phase B**~~ → **closed in Bref**: boot now seeds manifest-declared actors via synthetic `composition-change:add` events (full record on `payload.record`), and work-unit creation events carry the full record on `payload.after`. Pure `state_at(n)` replay reproduces both. The per-event check admits self-attestation for the on-event-added actor / work-unit so the chicken-and-egg at boot resolves cleanly. State mutations now flow only through `apply_event_to_state`.
