...
- **Last ledger entry**: **D74** (Phase C real-wire MCP server adapter; closes C6; closes Phase C closure item (b) "MCP server external invocation succeeds (C6 + D21 generalization)" per D68 §C — in-process `mcp.shared.memory.create_connected_server_and_client_session` self-test harness per scope-cut C12; validates D21 §generalization workspace-as-MCP-server + activates D60 publicly-exposed filter at server-side surface; MCPServerAdapter async-native (no threading; unlike c5) with tool-aggregator + tool-router + outbound NotImplementedError; mcp-server-side-ext 0.1.0 NEW protocol-or-transport identifier `mcp-server` DISTINCT from `mcp-server-ext` CLIENT-side family; HYBRID pure pattern application — c3 D71 MCP SDK UNCHANGED + c5 D73 server-side aggregator pattern UNCHANGED + c5 D73 §B.3 inbound-handler triad SHAPE REUSED translated to McpError surface; §B.1 inbound tool-call McpError vocabulary (-32601 method-not-found; -32603 internal-error); §B.2 server-bind REUSES D48 §B.2 adapter-attach unchanged; zero pyproject.toml dep delta — reuses mcp>=1.0 installed at D71; SKIPPED pre-lock probe per refined-skip rule (HYBRID pure pattern application; D72 SKIP precedent cluster); 11 new tests + 312 total).
- **Forward-bars locked at Phase B (activate in Phase C)**: D48 AdapterCallError **(ACTIVATED at D71 §B.1 under real-wire MCP transport + D72 §B.1 under real-wire HTTP transport + D73 §B.2 under A2A peer HTTP transport; starter category vocabulary mapped per D48 §D D-3 resolution — MCP and HTTP; native auth-category discrimination achieved at D72 §B.1 + D73 §B.2 for HTTP)**; D21 workspace-as-A2A-peer deployability **(ACTIVATED at D73 — A2APeerAdapter combined publisher + task-receiver + outbound peer-client)**; D21 §generalization workspace-as-MCP-server **(ACTIVATED at D74 — MCPServerAdapter async-native tool-aggregator + tool-router; closes Phase C closure item (b))**; D60 publicly-exposed slot **(ACTIVATED at D73 — agent-card aggregator + task-routing filter both honor the flag; D74 — tool-aggregator + tool-router both honor at MCP server-side surface)**; D50 SkillExecutionError; D54 classify_shape_change + shape-migration-unsafe **(ACTIVATED at D70 §B.2)**; D55-D59 §D D-7 Phase D end-to-end exercises; D56 §D D-5 practitioner-shape canonical constraints (D-7 closed by D65); D57 §D D-3 hot-reload; D58 §D D-5 snapshot caching **(D58 §B.1 reconciliation ACTIVATED at D70 §B.3; snapshot-caching strategy named explicitly as deferred per D70 §D D-1)**; D64 §D adapter emit-attribution + replay-validation per D40 + cross-specialist delegation + adapter.call outcomes + subclass override; D66 §D content-pattern checks + per-write-type required-Reads granularity + Phase C+ portability; **D69 §B.1 sdk-init** (NEW Phase C real-wire substrate forward-bar; Detection + Surface + Recovery per D45 triad — SDK init / auth / connection failures during substrate.start_session wrap as `WorkspaceBootError(category="sdk-init")` with Python exception chaining); **D70 §B.1 persistence-corruption** (NEW Phase C persistence forward-bar; Detection + Surface + Recovery per D45 triad — JSONL file IO / parse / partial-line failures during boot replay or manifest snapshot read wrap as `WorkspaceBootError(category="persistence-corruption")` with `PersistenceCorruptionError` chained via `from`).
