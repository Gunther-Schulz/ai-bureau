...
5. **Read `probing.md` in full** — adversarial probing discipline + investigation-before-claim discipline. **Mandatory session-start read** (promoted from on-demand 2026-05-12 — the on-demand trigger empirically failed: a sketch-without-grounding compliance failure occurred when the trigger condition fired but probing.md was not loaded). The discipline names what counts as Read-or-Cite vs unflagged inference and is load-bearing for ALL substantive work in this corpus, not just D-entry drafting. Cost of always-loading is bounded (~210 lines); cost of skipping when the trigger fires is unbounded (D47 §B.3 confabulation + the 2026-05-12 sketch-without-grounding incident).
