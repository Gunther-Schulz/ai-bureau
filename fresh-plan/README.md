...
5. **HARD RULE — Cite-or-Read-or-Flag for every load-bearing claim; Flag is TEMPORARY state for load-bearing claims; Honest-basis-caveats at end of substantive responses.** Every claim about code, state, file content, or what a document says MUST be backed by one of: **Cite** (specific file:line already Read in current session), **Read** (read source now, then assert), or **Flag** (explicitly tag as "inferred from adjacent signal X — not verified by reading source"). **Unflagged inference IS a violation** — not a soft preference. **Flag is a TEMPORARY state for load-bearing claims, never the final state.** For claims driving a decision / code change / recommendation / D-entry lock / impl follow-through, Flag MUST be converted to Read in the **same turn** (preferred) or **next turn** (acceptable). Accumulating unresolved Flags on load-bearing claims IS a violation, not a workaround. **Track Flagged-pending-Read state actively**: when you notice (mid-response or post-response) that you've Flagged something load-bearing that you should have Read, Read it same-or-next-turn and revise the assertion — don't let the Flag stand silently. Every substantive response (sketch, draft, analysis, recommendation) MUST close with an "Honest basis caveats" section naming what was Read directly vs claimed-but-not-verified vs inferred — AND explicitly listing any Flagged-pending-Read items that the next turn must resolve. **This is THE canonical AI failure mode** — not getting enough relevant context; pattern-completing from synthetic memory; claiming line numbers / function signatures / file content without verification; settling for Flag when Reading is cheap. The 2026-05-12 sketch-without-grounding incident is one instance; the failure pattern is general across AI assistants. Sources: global CLAUDE.md §"Honesty about sources" + `probing.md` §"Investigation-before-claim discipline". Pre-draft self-audit MANDATORY: before delivering a substantive output, list each load-bearing claim → tag Cite/Read/Flag → for any Flag on a load-bearing claim, READ now (same turn) or commit explicitly to Reading next turn. **Tracked structural backstop**: a PreToolUse hook (analog of inherited pbs-bureau `plugin/hooks/architectural_commit_gate.py`) is the deeper fix — prose rules empirically drift; hooks are deterministic per Anthropic engineering guidance. Tracked in `roadmap.md` as Bref deliverable #20.
6. **Confirm context tightly** — in 2–3 sentences, tell the user where work stands (last decision number; current phase per D26; any explicit open threads — including Phase B impl status at `fresh-plan/impl/` if relevant). Do not summarize the entire ledger; cite by entry number.
7. **Wait for user direction** — do not jump into work. The user may have specific next steps that don't follow the obvious path.
...
- **Current phase**: **Phase B** (reference impl of core). Impl side complete (B1-B8 + B2b; 183 tests pass post-D48). **Bref refinement workstream active**; processing the bounded-fill plan's cluster supersedes (D45 meta-foundation + D46 boot-procedure + D47 subscriber-dispatch + D48 adapter landed; D49-D51 remaining per roadmap deliverables 17-19). See `roadmap.md` Phase B section.
- **Last ledger entry**: **D48** (extends D16 — adapter cluster supersedes per D45 §C; first cluster supersedes to FIRE the pre-lock probe per probing.md refined-skip rule; 5 explicit Phase C deferrals named).
