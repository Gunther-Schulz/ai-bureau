...
- **Phase B**: **in progress.** B1 + B2 (+ followons) + B2b (MSAgentFrameworkSubstrate per D41; `Substrate` base extracted; D17 cross-tension confirmed → Bref) + B3 (Shape/GenericShape) + B4 (Adapter/MCPToolAdapter) + B5 (DirectAPIAdapter) + B6 (Specialist/GenericSpecialist; event-streaming push per D37) + B7 (RAGSpecialist + rag-via-mcp-ext per D38; demonstrates one-protocol-many-provisions architectural property) committed; 157 tests pass. Implementation discipline locked. boot.py step-3 + step-8 inline duplications both eliminated. D39 out-of-band tensions queued for Bref. **B8 (end-to-end) is next**, then Bref + closure entry.
...
- **Open threads at handoff**: B8 (end-to-end scenario per D36 closure trigger; next); **Bref (Phase B refinement workstream per D42)** with seven tracked deliverables (boot.py step-3 cleanup done in B2b; remaining: D39 out-of-band-state tensions / D17 capability-vocabulary sharpening *(confirmed by B2b — see CONCEPTS for the concept-name mapping)* / D38 worked-example validation / activation-scope DSL design / subscriber-dispatch reentrancy / migration-safety discipline for shape versioning / decisions.md split); Phase B closure entry (analog of D25 / D35); real-wire MCP / direct-api / MS Agent Framework integration deferred to Phase C+.
