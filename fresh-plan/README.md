...
- **Phase B**: **in progress.** B1 + B2 (+ followons) + B2b (MSAgentFrameworkSubstrate per D41; `Substrate` base extracted; D17 cross-tension on Claude-flavored capability naming confirmed → Bref) + B3 (Shape/GenericShape) + B4 (Adapter/MCPToolAdapter) + B5 (DirectAPIAdapter) + B6 (Specialist/GenericSpecialist; substrate event-streaming push semantics per D37) committed; 149 tests pass. Implementation discipline locked. boot.py step-3 + step-8 inline duplications both eliminated via `load_provision_spec`. D39 out-of-band tensions queued for end-of-Phase-B refinement. B7 (RAG-via-MCP) is next; B8 (end-to-end) last before Bref + closure entry.
...
- **Open threads at handoff**: B7 (RAG-via-MCP; next per D36); B8 (end-to-end); **Bref (Phase B refinement workstream per D42)** with seven tracked deliverables (boot.py step-3 cleanup done in B2b; remaining: D39 out-of-band-state tensions / D17 capability-vocabulary sharpening *(confirmed by B2b — see CONCEPTS for the concept-name mapping)* / D38 worked-example validation / activation-scope DSL design / subscriber-dispatch reentrancy / migration-safety discipline for shape versioning / decisions.md split); Phase B closure entry (analog of D25 / D35); real-wire MCP / direct-api / MS Agent Framework integration deferred to Phase C+.
