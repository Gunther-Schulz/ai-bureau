...
- **Last ledger entry**: **D41** (clarifies D26 + D36 — two-substrate parity required for Phase B closure; adds B2b MS Agent Framework substrate stub as a new workstream).
...
- **Open threads at handoff**: B2b (MS Agent Framework substrate stub per D41 — required for Phase B closure; will also surface whether to extract `Substrate` base earlier than originally planned); B7 (RAG-via-MCP); B8 (end-to-end); D39 out-of-band-state tensions (manifest-declared actors at boot; work-unit record carry); end-of-Phase-B refinement (per D14 / D34) — D17 capability vocabulary sharpening if B2b cross-tension surfaces + D38 worked-example validation + activation-scope DSL design (D19 slot currently opaque-metadata-only) + `decisions.md` split into per-entry files at `decisions/`; Phase B closure entry (analog of D25 / D35); real-wire MCP / direct-api integration deferred to Phase C+.
