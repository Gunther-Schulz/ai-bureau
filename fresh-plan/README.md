...
- **Open threads at handoff**: **Bref (Phase B refinement workstream per D42; next)** with seven tracked deliverables (canonical list in CONCEPTS open-questions: D39 out-of-band-state tensions / D17 capability-vocabulary sharpening *(confirmed by B2b)* / D38 worked-example validation / activation-scope DSL design / subscriber-dispatch reentrancy / migration-safety discipline for shape versioning / decisions.md split); Phase B closure entry (analog of D25 / D35); real-wire MCP / direct-api / MS Agent Framework integration deferred to Phase C+.
