...
- **Open threads at handoff**: B6 (specialist; next per D36 — Specialist base class written from the start per Implementation discipline); B2b (MS Agent Framework substrate stub per D41 — required for Phase B closure; parallel-eligible with B6/B7/B8; will also surface whether to extract `Substrate` base earlier than originally planned); B7 / B8; D39 out-of-band-state tensions (manifest-declared actors at boot; work-unit record carry); end-of-Phase-B refinement (per D14 / D34) — D17 capability vocabulary sharpening if B2b cross-tension surfaces; Phase B closure entry (analog of D25 / D35); real-wire MCP / direct-api integration deferred to Phase C+.
