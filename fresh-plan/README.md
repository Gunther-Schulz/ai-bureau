...
- **Last ledger entry**: **D76** (Phase C AEGIS integrity protocol; closes C8; closes Phase C closure item (g) "AEGIS integrity-chain verification round-trip" per D68 §C; ACTIVATES D40 §B integrity-mechanism extension point as canonical first concrete example; NEW module `runtime/integrity.py` (~526 LOC) with `IntegrityProtocol` ABC (D40 §B abstract — future Axon / PROV-O / post-quantum protocols register here per D29 namespacing) + `AEGISIntegrityProtocol` concrete (SHA-256 hash chain over JCS(event) || prev_hash via rfc8785 + Ed25519 per-event signing via cryptography; first-event prev_hash = SHA-256(workspace_id)) + `IntegrityRecord` dataclass + `IntegrityViolationError` typed exception + 6 keypair / PEM helpers; NEW `FAILURE_CATEGORIES` entry `integrity-violation` (21 → 22) covering hash-mismatch / signature-invalid / missing-record / canonicalization-error sub-cases (substructured via reason-string `subcase=` convention); NEW extension `aegis-protocol-ext/0.1.0/` registers vocabulary entry `adapter.protocol-or-transport: aegis-event-chain-integrity` per D29 + D40 §B (NO provisions — integrity protocols are protocol-or-transport identifiers selected via substrate-binding configuration); PersistenceLayer extension adds optional `integrity_protocol` field + `events.integrity.jsonl` parallel-line side-effect + `has_integrity_records()` + `load_integrity_records()` + `prime_integrity_state()` methods (+204 LOC net); boot.py step 4.5+ integration between persistence-replay and shape-migration check dispatches by `substrate-bindings[0].configuration.integrity-protocol` per D57 §B.1 opaque pass-through; per-workspace Ed25519 keypair persists as `integrity-private-key.pem` (mode 600) + `integrity-public-key.pem` next to events.jsonl; NEW pyproject deps `cryptography>=42` (was transitive via claude-agent-sdk; now declared direct per V1 dependency hygiene) + `rfc8785>=0.1`; FIRED pre-lock probe per refined-skip rule (NEW: abstract base + concrete impl + dataclass + typed exception + FAILURE_CATEGORIES entry + protocol-or-transport identifier + extension + persistence side-effect + boot-step integration + 2 deps); 6 §D defers cover Phase D+ refinements (other integrity protocols / production key management KMS-HSM / EU AI Act Article 12 audit-record-bundle format per D24 + D68 §D external-trigger 2026-08-02 / multi-protocol coexistence / rollback on integrity-record write failure / encrypted private keys); 10 new tests + 332 total).
