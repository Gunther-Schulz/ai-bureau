...
- **Last ledger entry**: **D70** (Phase C JSONL persistence layer; closes C2; activates D54 §B.2 classify_shape_change boot integration + D58 §B.1 lifecycle-derivation reconciliation; FIRED pre-lock probe per refined-skip rule for new contract — `persistence-corruption` FAILURE_CATEGORIES entry + PersistenceLayer module + boot step 4.5 ordering; 13 new tests + 253 total).
- **Forward-bars locked at Phase B (activate in Phase C)**: D48 AdapterCallError; D50 SkillExecutionError; D54 classify_shape_change + shape-migration-unsafe **(ACTIVATED at D70 §B.2)**; D55-D59 §D D-7 Phase D end-to-end exercises; D56 §D D-5 practitioner-shape canonical constraints (D-7 closed by D65); D57 §D D-3 hot-reload; D58 §D D-5 snapshot caching **(D58 §B.1 reconciliation ACTIVATED at D70 §B.3; snapshot-caching strategy named explicitly as deferred per D70 §D D-1)**; D64 §D adapter emit-attribution + replay-validation per D40 + cross-specialist delegation + adapter.call outcomes + subclass override; D66 §D content-pattern checks + per-write-type required-Reads granularity + Phase C+ portability; **D69 §B.1 sdk-init** (NEW Phase C real-wire substrate forward-bar; Detection + Surface + Recovery per D45 triad — SDK init / auth / connection failures during substrate.start_session wrap as `WorkspaceBootError(category="sdk-init")` with Python exception chaining); **D70 §B.1 persistence-corruption** (NEW Phase C persistence forward-bar; Detection + Surface + Recovery per D45 triad — JSONL file IO / parse / partial-line failures during boot replay or manifest snapshot read wrap as `WorkspaceBootError(category="persistence-corruption")` with `PersistenceCorruptionError` chained via `from`).
