...
5. **Apply Cite-or-Read-or-Flag + Honest-basis-caveats throughout the session — behavioral discipline gate.** Every claim about code, state, file content, or what a document says MUST be backed by one of: **Cite** (specific file:line already Read in current session), **Read** (read source now, then assert), or **Flag** (explicitly tag as "inferred from adjacent signal X — not verified by reading source"). Every substantive response (sketch, draft, analysis, recommendation) MUST close with an "Honest basis caveats" section naming what was Read directly vs claimed-but-not-verified vs inferred. Sources: global CLAUDE.md §"Honesty about sources" + `probing.md` §"Investigation-before-claim discipline". This is a **behavioral** discipline (no structural enforcement here); codified at session-start after the 2026-05-12 sketch-without-grounding incident where the discipline content was available (global CLAUDE.md + prior-session probing.md references) but not applied at claim-time. Pre-draft self-audit recommended: before delivering a substantive output, list each load-bearing claim → tag Cite/Read/Flag → revise unflagged claims to Read or Flag.
