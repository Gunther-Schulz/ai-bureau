...
- **Phase B**: **in progress.** B1 + B2 + B2-followon-1 (D39 `record`) + B2-followon-2 (D40 §A `state_at(n)`) + B3 (generic minimal shape impl per D13) + B4 (stub MCP-server adapter per D16 + D29 protocol-identifier registration) committed; 128 tests pass. D39 out-of-band-state tensions documented for end-of-Phase-B refinement (manifest-declared actors at boot; work-unit records carried only by id). B5 (direct-api adapter) is next per D36 workstream order; B4/B5 can run in parallel.
...
- **Open threads at handoff**: B5 (direct-api adapter; next per D36); B6 / B7 / B8; D39 out-of-band-state tensions (manifest-declared actors at boot; work-unit record carry); end-of-Phase-B refinement (per D14 / D34); Phase B closure entry (analog of D25 / D35); real-wire MCP integration deferred to Phase C (B4 currently stub).
