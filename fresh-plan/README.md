...
- **Last ledger entry**: **D42** (clarifies D14 + D36 — formalizes Bref as named Phase B refinement workstream running between B8 and Phase B closure entry; output D-entry analogous to D34).
- **Phase A**: closed at D35; refined by D34 + sharpened by side-quest batch D37-D40.
- **Phase B**: **in progress.** B1 + B2 (+ followons) + B3 (Shape/GenericShape) + B4 (Adapter/MCPToolAdapter) + B5 (DirectAPIAdapter) + B6 (Specialist/GenericSpecialist per D19; substrate gained subscriber-dispatch infrastructure giving D17 `event-streaming` real push semantics; boot.py step 8 inline duplication eliminated via `load_provision_spec`) committed; 142 tests pass. Implementation discipline locked (contract-driven abstraction; no sunk cost) — base class from the start for every kind impl. D39 out-of-band-state tensions remain queued for end-of-Phase-B refinement. B2b (MS Agent Framework substrate stub per D41) and B7 (RAG-via-MCP) parallel-eligible; B8 (end-to-end) last.
...
- **Open threads at handoff**: B2b (MS Agent Framework substrate stub per D41 — required for Phase B closure; will also surface whether to extract `Substrate` base earlier than originally planned); B7 (RAG-via-MCP); B8 (end-to-end); **Bref (Phase B refinement workstream per D42; runs between B8 and Phase B closure entry)** with eight tracked deliverables (canonical list in `CONCEPTS.md` open-questions: D39 out-of-band-state tensions / D17 capability-vocabulary sharpening / D38 worked-example validation / activation-scope DSL design / boot.py step-3 cleanup / subscriber-dispatch reentrancy / migration-safety discipline for shape versioning / decisions.md split); Phase B closure entry (analog of D25 / D35); real-wire MCP / direct-api integration deferred to Phase C+.
