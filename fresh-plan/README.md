...
- **Last ledger entry**: **D73** (Phase C real-wire A2A peer adapter; closes C5; closes Phase C closure item (a) "A2A peer external interaction succeeds (C5 + D21)" per D68 §C — in-process ThreadingHTTPServer + httpx.Client harness per scope-cut C12; validates D21 workspace-as-A2A-peer deployability + activates D60 publicly-exposed filter; A2APeerAdapter combined publisher + task-receiver + outbound peer-client; a2a-peer-ext 0.1.0 NEW protocol-or-transport identifier; server-async lifecycle via stdlib http.server.ThreadingHTTPServer on daemon threading.Thread (HTTP-transport-local, NOT substrate-level event loop per D44); §B.1 server-bind REUSES D48 §B.2 adapter-attach category; §B.2 outbound REUSES D72 §B.1 starter category vocabulary including NATIVE 401/403/407 auth-category mapping; §B.3 NEW inbound-handler triad with HTTP error response surface; a2a-sdk 1.0.3 canonical protobuf AgentCard/AgentSkill types serialized via google.protobuf.json_format; FIRED pre-lock probe per refined-skip rule (5 NEW contract elements); 15 new tests + 301 total).
- **Forward-bars locked at Phase B (activate in Phase C)**: D48 AdapterCallError **(ACTIVATED at D71 §B.1 under real-wire MCP transport + D72 §B.1 under real-wire HTTP transport + D73 §B.2 under A2A peer HTTP transport; starter category vocabulary mapped per D48 §D D-3 resolution — MCP and HTTP; native auth-category discrimination achieved at D72 §B.1 + D73 §B.2 for HTTP)**; D21 workspace-as-A2A-peer deployability **(ACTIVATED at D73 — A2APeerAdapter combined publisher + task-receiver + outbound peer-client)**; D60 publicly-exposed slot **(ACTIVATED at D73 — agent-card aggregator + task-routing filter both honor the flag)**; D50 SkillExecutionError; D54 classify_shape_change + shape-migration-unsafe **(ACTIVATED at D70 §B.2)**; D55-D59 §D D-7 Phase D end-to-end exercises; D56 §D D-5 practitioner-shape canonical constraints (D-7 closed by D65); D57 §D D-3 hot-reload; D58 §D D-5 snapshot caching **(D58 §B.1 reconciliation ACTIVATED at D70 §B.3; snapshot-caching strategy named explicitly as deferred per D70 §D D-1)**; D64 §D adapter emit-attribution + replay-validation per D40 + cross-specialist delegation + adapter.call outcomes + subclass override; D66 §D content-pattern checks + per-write-type required-Reads granularity + Phase C+ portability; **D69 §B.1 sdk-init** (NEW Phase C real-wire substrate forward-bar; Detection + Surface + Recovery per D45 triad — SDK init / auth / connection failures during substrate.start_session wrap as `WorkspaceBootError(category="sdk-init")` with Python exception chaining); **D70 §B.1 persistence-corruption** (NEW Phase C persistence forward-bar; Detection + Surface + Recovery per D45 triad — JSONL file IO / parse / partial-line failures during boot replay or manifest snapshot read wrap as `WorkspaceBootError(category="persistence-corruption")` with `PersistenceCorruptionError` chained via `from`).
