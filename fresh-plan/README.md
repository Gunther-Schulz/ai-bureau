...
- **Last ledger entry**: **D75** (Phase C real-wire RAG specialist; closes C7; closes Phase C closure items (c) "real-wire RAG-via-MCP end-to-end" + (f) "SkillExecutionError per starter category" per D68 §C; ACTIVATES D50 §B.1 SkillExecutionError forward-bar under real-wire conditions; RESOLVES D50 §D D-5 composition contract as first concrete precedent — WRAP AdapterCallError as SkillExecutionError(category='external-dependency-error') with `__cause__` chain preserving original AdapterCallError per `raise ... from exc`; NEW RealWireRAGSpecialist class subclassing Specialist (replaces B7 RAGSpecialist stub; B7 stub preserved per D41 two-version parity) + rag-via-mcp-ext 0.2.0 NEW specialist provision `rag-realwire-specialist` with `required-adapter-bindings=["mcp-server-ext:mcp-real-wire-adapter"]` (the c3 D71 real-wire adapter provision); 4-category dispatch ladder per D50 §B.1 starter vocabulary inside handle_skill body (domain-error params validation BEFORE action emit / external-dependency-error AdapterCallError caught + WRAPped / skill-execution result-parse invariant violations / unknown bare-except catch-all); D64 §B.1 emit-attribution INHERITED via Specialist.attach_workspace closure-wrap UNCHANGED (tests verify emitting-specialist field carries binding-id); pre-wire action emit per D71 §B.1 convention; in-process MCP server harness via `mcp.shared.memory.create_connected_server_and_client_session` per scope-cut C12 (in-process `retrieve` tool returning canned TextContent blocks); zero pyproject.toml dep delta — reuses mcp>=1.0 installed at D71 + c3 RealWireMCPClientAdapter UNCHANGED; FIRED pre-lock probe per refined-skip rule (NEW contract: D50 §B.1 ACTIVATION + D50 §D D-5 RESOLUTION first concrete precedent + cross-typed-exception flow end-to-end); 5 §D defers cover Phase C+ refinements (real-corpus retrieval / per-shape category vocabulary / streaming + batch + federation / specialist-side caching / generic-specialist real-wire equivalent); 10 new tests + 322 total).
- **Forward-bars locked at Phase B (activate in Phase C)**: D48 AdapterCallError **(ACTIVATED at D71 §B.1 under real-wire MCP transport + D72 §B.1 under real-wire HTTP transport + D73 §B.2 under A2A peer HTTP transport; starter category vocabulary mapped per D48 §D D-3 resolution — MCP and HTTP; native auth-category discrimination achieved at D72 §B.1 + D73 §B.2 for HTTP)**; D21 workspace-as-A2A-peer deployability **(ACTIVATED at D73 — A2APeerAdapter combined publisher + task-receiver + outbound peer-client)**; D21 §generalization workspace-as-MCP-server **(ACTIVATED at D74 — MCPServerAdapter async-native tool-aggregator + tool-router; closes Phase C closure item (b))**; D60 publicly-exposed slot **(ACTIVATED at D73 — agent-card aggregator + task-routing filter both honor the flag; D74 — tool-aggregator + tool-router both honor at MCP server-side surface)**; D50 SkillExecutionError **(ACTIVATED at D75 §B.1 — RealWireRAGSpecialist handle_skill body 4-category dispatch ladder under real-wire MCP retrieval flow; D50 §D D-5 composition contract RESOLVED as first concrete precedent: WRAP AdapterCallError with __cause__ chain)**; D54 classify_shape_change + shape-migration-unsafe **(ACTIVATED at D70 §B.2)**; D55-D59 §D D-7 Phase D end-to-end exercises; D56 §D D-5 practitioner-shape canonical constraints (D-7 closed by D65); D57 §D D-3 hot-reload; D58 §D D-5 snapshot caching **(D58 §B.1 reconciliation ACTIVATED at D70 §B.3; snapshot-caching strategy named explicitly as deferred per D70 §D D-1)**; D64 §D adapter emit-attribution + replay-validation per D40 + cross-specialist delegation + adapter.call outcomes + subclass override; D66 §D content-pattern checks + per-write-type required-Reads granularity + Phase C+ portability; **D69 §B.1 sdk-init** (NEW Phase C real-wire substrate forward-bar; Detection + Surface + Recovery per D45 triad — SDK init / auth / connection failures during substrate.start_session wrap as `WorkspaceBootError(category="sdk-init")` with Python exception chaining); **D70 §B.1 persistence-corruption** (NEW Phase C persistence forward-bar; Detection + Surface + Recovery per D45 triad — JSONL file IO / parse / partial-line failures during boot replay or manifest snapshot read wrap as `WorkspaceBootError(category="persistence-corruption")` with `PersistenceCorruptionError` chained via `from`).
