...
- **Phase B**: **impl side complete; Bref + closure entry remaining.** B1 + B2 (+ followons) + B2b + B3 + B4 + B5 + B6 + B7 + B8 (end-to-end scenario; D36 §C closure-criterion items 1–6 satisfied) committed; 164 tests pass. Implementation discipline locked. All planned base classes extracted (Shape, Adapter, Specialist, Substrate). All planned vocabulary-registrations landed (work-unit.kind, adapter.protocol-or-transport). D39 out-of-band tensions + D17 capability-vocabulary sharpening + other Bref deliverables queued. **Bref (refinement workstream per D42) is next**, then Phase B closure entry (analog of D25 / D35).
...
- **Open threads at handoff**: **Bref (Phase B refinement workstream per D42; next)** with seven tracked deliverables (canonical list in `CONCEPTS.md` open-questions: D39 out-of-band-state tensions / D17 capability-vocabulary sharpening *(confirmed by B2b — concept-name mapping in CONCEPTS)* / D38 worked-example validation / activation-scope DSL design / subscriber-dispatch reentrancy / migration-safety discipline for shape versioning / `decisions.md` split). Bref output = one substantive D-entry analogous to **D34** (Phase A's refinement output — read it as structural template). Then **Phase B closure entry** analogous to **D35** (Phase A's closure — read it as structural template: artifact inventory + closure-criterion satisfaction + deferrals). Real-wire MCP / direct-api / MS Agent Framework integration deferred to Phase C+.
