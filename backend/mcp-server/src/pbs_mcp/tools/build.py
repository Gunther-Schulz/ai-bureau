...
def _generate_identity_macros(cfg) -> str:
    """Render office-identity.tex from the office section of office-config.

    v3: identity fields live directly under cfg.office (merged from v2's
    cfg.identity). Default signer comes from the first internal actor.
    """
    o = cfg.office
    addr_lines = "".join(f"  {_escape_tex(line)}\\\\\n" for line in o.address_lines)
    lines = [
        ...
        "",
        f"\\providecommand{{\\OfficeName}}{{{_escape_tex(o.name)}}}",
        f"\\providecommand{{\\OfficeShort}}{{{_escape_tex(o.short)}}}",
        "\\providecommand{\\OfficeAddressLines}{%",
        addr_lines.rstrip(),
        "}",
    ]
    if o.title:
        lines.append(f"\\providecommand{{\\OfficeTitle}}{{{_escape_tex(o.title)}}}")
    if o.phone:
        lines.append(f"\\providecommand{{\\OfficePhone}}{{{_escape_tex(o.phone)}}}")
    if o.mobile:
        lines.append(f"\\providecommand{{\\OfficeMobile}}{{{_escape_tex(o.mobile)}}}")
    if o.fax:
        lines.append(f"\\providecommand{{\\OfficeFax}}{{{_escape_tex(o.fax)}}}")
    if o.email:
        lines.append(f"\\providecommand{{\\OfficeEmail}}{{{_escape_tex(o.email)}}}")
    if o.web:
        lines.append(f"\\providecommand{{\\OfficeWeb}}{{{_escape_tex(o.web)}}}")
    if o.specializations:
        spec_lines = "".join(f"  {_escape_tex(s)}\\\\\n" for s in o.specializations)
    ...
    if o.logo_path:
        ...
        lines.append(f"\\providecommand{{\\OfficeLogoPath}}{{{str(o.logo_path).replace(chr(92), '/')}}}")
    if o.signature_image_path:
        lines.append(f"\\providecommand{{\\OfficeSignaturePath}}{{{str(o.signature_image_path).replace(chr(92), '/')}}}")
    # Escape per-line first, THEN join with LaTeX line breaks, so the
    # LaTeX line-break syntax doesn't get double-escaped.
    escaped_sig_lines = [_escape_tex(line) for line in o.signature_block.splitlines() if line.strip()]
    sig = "\\\\\n".join(escaped_sig_lines)
    lines.append(f"\\providecommand{{\\OfficeSignatureBlock}}{{%\n{sig}}}")
    default = cfg.default_internal_actor()
