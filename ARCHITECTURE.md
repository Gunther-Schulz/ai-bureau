...
| **3.5** | Per-primitive detail topics (4 primitive-cluster topics + 2 cross-cutting integrators) | ACTIVE — second primitive-cluster topic LOCKED (practitioner; Pattern C topic-template-class anchor); 4 primitive-cluster + cross-cutting integrator topics remain (workflow-work-unit + claim-defensibility primitive clusters + scope-model + axis-interactions cross-cutting integrators) |
...
| `arch/<topic-slug>.md` × 11 | Per-topic detail | ~500 each | Active (6 of 11 drafted: substrate / adapter / sparring / audit / specialist-skill / practitioner) |
